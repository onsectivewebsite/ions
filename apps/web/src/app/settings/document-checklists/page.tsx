...
import { ArrowLeft, FileCheck, Pencil, Plus } from 'lucide-react';
...
export default function DocumentChecklistsListPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      rpcQuery<Template[]>('documentChecklistTemplate.list', undefined, { token }),
    ])
      .then(([m, list]) => {
        if (m.kind !== 'firm') {
  ...
  return (
    <ThemeProvider branding={me.tenant.branding ?? { themeCode: 'maple' }}>
      <AppShell
        user={{ name: me.name, email: me.email, scope: 'firm', contextLabel: me.tenant.displayName }}
      >
        <div className="mx-auto w-full max-w-4xl space-y-6">
          <div className="flex items-end justify-between">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-semibold tracking-tight">Document checklists</h1>
              <p className="mt-1 text-sm text-[var(--color-text-muted)]">
                Per-case-type lists of documents the client must upload. The system picks the
                most specific active checklist when a case enters PENDING_DOCUMENTS. Without a
                template, a sensible Canadian-immigration default is used.
              </p>
            </div>
            <Link href="/settings/document-checklists/new">
              <Button>
          ...
          <Card>
            {items.length === 0 ? (
              <div className="py-12 text-center text-sm text-[var(--color-text-muted)]">
                ...
                No checklists yet. The built-in default will be used until you add one.
              </div>
            ) : (
              <ul className="divide-y divide-[var(--color-border-muted)]">
                {items.map((t) => {
                  const itemCount = Array.isArray(t.itemsJson)
                    ? (t.itemsJson as unknown[]).length
                    : 0;
                  return (
                    <li key={t.id}>
                      <Link
                        href={`/settings/document-checklists/${t.id}`}
