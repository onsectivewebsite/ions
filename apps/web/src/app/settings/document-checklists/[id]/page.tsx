...
} from '../../../../components/documents/ChecklistEditor';
...
export default function EditChecklistPage() {
  ...
  const [initial, setInitial] = useState<ChecklistTemplateState | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      rpcQuery<TemplateRow>('documentChecklistTemplate.get', { id: params.id }, { token }),
    ])
      .then(([m, t]) => {
        if (m.kind !== 'firm') {
  ...
  return (
    <ThemeProvider branding={me.tenant.branding ?? { themeCode: 'maple' }}>
      <AppShell user={{ name: me.name, email: me.email, scope: 'firm', contextLabel: me.tenant.displayName }}>
        <ChecklistEditor initial={initial} />
      </AppShell>
    </ThemeProvider>
  );
}
